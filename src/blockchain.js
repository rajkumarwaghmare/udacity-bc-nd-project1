...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    let self = this;
    return new Promise(async (resolve, reject) => {
      let messageSentTime = message.split(":")[1];
      let currentTime = parseInt(new Date().getTime().toString().slice(0, 3));
      let timeLapsedInSec = currentTime - messageSentTime;
      if (timeLapsedInSec >= 300) {
        //5min = 300sec
        reject("5min passed!");
      }
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      let block = self.chain.find((block) => block.hash === hash); //find is more efficient than filter
      if (block) {
        resolve(block);
      } else {
        resolve(null);
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise(async (resolve, reject) => {
      for (const block of self.chain) {
        //Linear search of BC will be inefficient when BC grows in size!
